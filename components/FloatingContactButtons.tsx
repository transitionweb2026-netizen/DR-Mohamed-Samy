...
export type FloatingContactButtonsContent = {
  phone: string;
  whatsappHref: string;
  callUsAriaLabel: string;
  chatOnWhatsappAriaLabel: string;
};
...
export default function FloatingContactButtons({ content }: { content: FloatingContactButtonsContent }) {
  const { phone, whatsappHref, callUsAriaLabel, chatOnWhatsappAriaLabel } = content;
  const telHref = `tel:${phone.replace(/[^+\d]/g, "")}`;

  return (
    <div className="fixed bottom-5 start-5 md:bottom-8 md:start-8 z-40 flex flex-col gap-3">
      <a
        aria-label={callUsAriaLabel}
      ...
      <a
        aria-label={chatOnWhatsappAriaLabel}
